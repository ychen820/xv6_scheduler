...
scheduler(void)
{
  ...
  if(sched_type)
   {
   /*  struct proc *high_p[NPROC]={0};
     struct proc *mid_p[NPROC]={0};
     struct proc *low_p[NPROC]={0};
     int high_index=0;
     int mid_index=0;
     int low_index=0;
    */
 ...
 for(p =ptable.proc;p < &ptable.proc[NPROC];  p++)
{
  ...
  // cprintf("max_priority:%d,max_pid:%d\n",max_priority,max_pid);
 //  cprintf("enter priority:%d,pid:%d\n",p->priority,p->pid);
// cprintf("%d",high_p[high_index-1])
